package org.example.ui;

import org.example.faculty.FacultyController;
import org.example.faculty.dto.FacultyDto;
import org.example.tui.Frame;
import org.example.tui.Screen;
import org.example.tui.UiContext;
import org.example.tui.input.InputEvent;
import org.example.tui.input.Key;
import org.example.tui.input.KeyStroke;
import org.example.tui.widget.*;
import org.jline.utils.AttributedStyle;

import java.util.ArrayList;
import java.util.List;

public final class FacultyScreen implements Screen {
    private final FacultyController facultyController;

    public FacultyScreen(FacultyController facultyController) {
        this.facultyController = facultyController;
    }

    @Override
    public int zIndex() {
        return 0;
    }

    @Override
    public boolean blocksInput() {
        return false;
    }

    @Override
    public boolean onInput(InputEvent event, UiContext ctx) {
        if (event instanceof InputEvent.Key(KeyStroke stroke)) {
            if (stroke.key() instanceof Key.Named n && n == Key.Named.Escape) {
                ctx.host().pop();
                ctx.host().requestRedraw();
                return true;
            }

            if (stroke.key() instanceof Key.Named n && n == Key.Named.Enter) {
                editSelected(ctx);
                return true;
            }

            if (stroke.key() instanceof Key.Char(int cp)) {
                if (cp == 'n' || cp == 'N') {
                    createNew(ctx);
                    return true;
                }
                if (cp == 'd' || cp == 'D') {
                    deleteSelected(ctx);
                    return true;
                }
            }
        }

        TableWidget table = buildTable(ctx);
        if (table.onInput(event, ctx)) {
            ctx.host().requestRedraw();
            return true;
        }

        return false;
    }

    @Override
    public void render(Frame frame, UiContext ctx) {
        new Breadcrumbs(List.of("University", "Faculty"), AttributedStyle.DEFAULT.bold())
                .render(new Canvas(frame, new Rect(0, 0, frame.width(), 1)), ctx);

        Rect body = new Rect(0, 1, frame.width(), Math.max(0, frame.height() - 2));
        new Block("Faculty", buildTable(ctx), AttributedStyle.DEFAULT, AttributedStyle.DEFAULT.bold())
                .render(new Canvas(frame, body), ctx);

        new Paragraph("N = new, Enter = edit, D = delete, Esc = back", AttributedStyle.DEFAULT.faint())
                .render(new Canvas(frame, new Rect(0, frame.height() - 1, frame.width(), 1)), ctx);
    }

    private TableWidget buildTable(UiContext ctx) {
        List<FacultyDto> items = facultyController.listFaculty();
        List<List<String>> rows = new ArrayList<>();
        for (FacultyDto f : items) {
            rows.add(List.of(
                    f.fullName(),
                    f.email() == null ? "" : f.email(),
                    f.department() == null ? "" : f.department()
            ));
        }

        return new TableWidget(
                "uni.faculty.table",
                List.of("Name", "Email", "Dept"),
                rows,
                AttributedStyle.DEFAULT.bold(),
                AttributedStyle.DEFAULT,
                AttributedStyle.DEFAULT.inverse()
        );
    }

    private FacultyDto selected(UiContext ctx) {
        List<FacultyDto> items = facultyController.listFaculty();
        if (items.isEmpty()) {
            return null;
        }
        int sel = ctx.state().getInt("uni.faculty.table.sel", 0);
        if (sel < 0) sel = 0;
        if (sel >= items.size()) sel = items.size() - 1;
        return items.get(sel);
    }

    private void createNew(UiContext ctx) {
        ctx.host().push(UiOverlays.textInput("New faculty", "Full name:", "", (ui, fullName0) -> {
            String fullName = (fullName0 == null) ? "" : fullName0.trim();
            if (fullName.isEmpty()) {
                ui.host().push(UiOverlays.message("Error", "Full name is required."));
                ui.host().requestRedraw();
                return;
            }

            ui.host().push(UiOverlays.textInput("New faculty", "Email (optional):", "", (ui2, email0) -> {
                String email = (email0 == null) ? "" : email0.trim();

                ui2.host().push(UiOverlays.textInput("New faculty", "Department:", "", (ui3, dept0) -> {
                    String dept = (dept0 == null) ? "" : dept0.trim();
                    if (dept.isEmpty()) {
                        ui3.host().push(UiOverlays.message("Error", "Department is required."));
                        ui3.host().requestRedraw();
                        return;
                    }

                    try {
                        facultyController.createFaculty(fullName, email, dept);
                    } catch (RuntimeException e) {
                        ui3.host().push(UiOverlays.message("Error", String.valueOf(e.getMessage())));
                    }
                    ui3.host().requestRedraw();
                }));

                ui2.host().requestRedraw();
            }));

            ui.host().requestRedraw();
        }));
        ctx.host().requestRedraw();
    }

    private void editSelected(UiContext ctx) {
        FacultyDto f = selected(ctx);
        if (f == null) {
            ctx.host().push(UiOverlays.message("Edit", "No selection."));
            ctx.host().requestRedraw();
            return;
        }

        ctx.host().push(UiOverlays.textInput("Edit faculty", "Full name:", f.fullName(), (ui, fullName0) -> {
            String fullName = (fullName0 == null) ? "" : fullName0.trim();
            if (fullName.isEmpty()) {
                ui.host().push(UiOverlays.message("Error", "Full name is required."));
                ui.host().requestRedraw();
                return;
            }

            ui.host().push(UiOverlays.textInput("Edit faculty", "Email (optional):", f.email() == null ? "" : f.email(), (ui2, email0) -> {
                String email = (email0 == null) ? "" : email0.trim();

                ui2.host().push(UiOverlays.textInput("Edit faculty", "Department:", f.department() == null ? "" : f.department(), (ui3, dept0) -> {
                    String dept = (dept0 == null) ? "" : dept0.trim();
                    if (dept.isEmpty()) {
                        ui3.host().push(UiOverlays.message("Error", "Department is required."));
                        ui3.host().requestRedraw();
                        return;
                    }

                    try {
                        facultyController.updateFaculty(f.id(), fullName, email, dept);
                    } catch (RuntimeException e) {
                        ui3.host().push(UiOverlays.message("Error", String.valueOf(e.getMessage())));
                    }
                    ui3.host().requestRedraw();
                }));

                ui2.host().requestRedraw();
            }));

            ui.host().requestRedraw();
        }));
        ctx.host().requestRedraw();
    }

    private void deleteSelected(UiContext ctx) {
        FacultyDto f = selected(ctx);
        if (f == null) {
            ctx.host().push(UiOverlays.message("Delete", "No selection."));
            ctx.host().requestRedraw();
            return;
        }

        ctx.host().push(UiOverlays.confirm(
                "Delete faculty",
                "Delete faculty:\n" + f.fullName() + " (" + f.id() + ") ?",
                (ui, ok) -> {
                    if (!ok) {
                        return;
                    }
                    try {
                        facultyController.deleteFaculty(f.id());
                    } catch (RuntimeException e) {
                        ui.host().push(UiOverlays.message("Error", String.valueOf(e.getMessage())));
                    }
                    ui.host().requestRedraw();
                }
        ));
        ctx.host().requestRedraw();
    }
}
